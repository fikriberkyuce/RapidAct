...
export function ChatView({
    ...
    // US case-law research is opt-in (Account > Features). Even if a message
    // has case citations from before the toggle was turned off, don't let
    // the user open the CourtListener panel while the feature is disabled.
    const legalResearchUsEnabled = profile?.legalResearchUs === true;
    ...
    const openCitation = useCallback(
        (citation: CitationAnnotation, options?: { showQuotes?: boolean }) => {
            ...
            if (citation.kind === "case") {
                if (!chatId || !legalResearchUsEnabled) return;
                upsertTab({
                    kind: "case",
        ...
        [chatId, upsertTab, legalResearchUsEnabled],
    );
