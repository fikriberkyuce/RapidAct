...
function toolCallLabel(name: string): string {
    if (name === "generate_docx") return "Belge oluşturuluyor...";
    if (name === "edit_document") return "Belge düzenleniyor...";
    if (name === "read_document") return "Belge okunuyor...";
    if (name === "fetch_documents") return "Belgeler okunuyor...";
    if (name === "find_in_document") return "Belgede aranıyor...";
    if (name === "replicate_document") return "Belge kopyalanıyor...";
    if (name === "read_workflow") return "İş akışı yükleniyor...";
    if (name === "list_workflows") return "İş akışları yükleniyor...";
    if (name === "list_documents") return "Belgeler yükleniyor...";
    if (name === "courtlistener_search_case_law")
        return "İçtihat aranıyor...";
    if (name === "courtlistener_get_cases") return "Kararlar getiriliyor...";
    if (name === "courtlistener_find_in_case") return "Kararda aranıyor...";
    if (name === "courtlistener_read_case") return "Karar okunuyor...";
    if (name === "courtlistener_verify_citations")
        return "Atıflar doğrulanıyor...";
    if (name.startsWith("mcp_")) return "Bağlantı kullanılıyor...";
    return name ? `${name} çalıştırılıyor...` : "Çalışıyor...";
}
            ...
            for (const { annotation } of pending) {
                ...
                try {
                    ...
                    onError?.({
                        ...
                        message:
                            verb === "accept"
                                ? "Bir veya daha fazla kabul kaydedilemedi."
                                : "Bir veya daha fazla ret kaydedilemedi.",
                    });
                }
                done++;
...
function EditCardsSection({
    ...
    const summary =
        pending.length > 0
            ? docCount > 1
                ? `${docCount} belgede ${pending.length} izlenen değişiklik`
                : `${pending.length} izlenen değişiklik`
            : docCount > 1
              ? `${docCount} belgede ${resolvedCount} çözümlenen izlenen değişiklik`
              : `${resolvedCount} çözümlenen izlenen değişiklik`;
        ...
        <div className="border border-gray-200 rounded-lg bg-white overflow-hidden">
            ...
            <div className="flex items-center gap-2 px-3 pt-3">
                ...
                <button
                    ...
                    aria-label={isOpen ? "Değişiklikleri daralt" : "Değişiklikleri genişlet"}
                ...
                <div className="mt-2 ml-[14px]">
                        ...
                        {isCollapsed && (
                            <>
                                ...
                                <button
                                    ...
                                    aria-label="Düşünce sürecini genişlet"
                                >
                    ...
                    {isOverflowing && isContentOpen && isExpanded && (
                        <button
                            ...
                            aria-label="Düşünce sürecini küçült"
                        >
...
function DocFindBlock({
    ...
    const label = isStreaming ? "Aranıyor" : "Bulundu";
    const matchSuffix = isStreaming
        ? ""
        : ` (${totalMatches} ${totalMatches === 1 ? "eşleşme" : "eşleşme"})`;
    return (
        <div className="flex items-start text-sm font-serif text-gray-500 relative">
            {showConnector && (
                <div className="absolute bottom-0 w-[1px] bg-gray-300 top-[13px] left-[2.5px] h-[calc(100%+11px)]" />
...
function DocCreatedBlock({
    ...
    return (
        <div className="flex items-start text-sm font-serif text-gray-500 relative">
            ...
            <div className="ml-2 min-w-0 flex-1 whitespace-normal break-words">
                <span className="font-medium">
                    {isStreaming ? "Oluşturuluyor" : "Oluşturuldu"}
                </span>{" "}
...
function DocReplicatedBlock({
    ...
    const label = isStreaming ? "Çoğaltılıyor" : "Çoğaltıldı";
    const suffix =
        !isStreaming && count > 1
            ? ` ${count} kez`
            : isStreaming
...
function DocDownloadBlock({
    ...
    const ext = extMatch ? extMatch[1].toLocaleUpperCase("tr-TR") : "DOSYA";
...
function WorkflowAppliedBlock({
    ...
    return (
        <div className="flex items-start text-sm font-serif text-gray-500 relative">
            ...
            <div className="mt-2 w-1.5 h-1.5 rounded-full bg-green-400 shrink-0" />
            <div className="ml-2 min-w-0 flex-1 whitespace-normal break-words">
                <span className="font-medium">Uygulanan İş Akışı</span>{" "}
                {onClick ? (
                    <button
                        onClick={onClick}
                        className="text-left hover:text-gray-700 transition-colors cursor-pointer"
                ...
                <ul className="mt-2 ml-[14px] flex flex-col gap-1 text-sm font-serif text-gray-500">
                    {items!.map((item, idx) => {
                        const label = [item.caseName, item.citation]
                            .filter(Boolean)
                            .join(", ");
                        const primary = label || item.url || "Bilinmeyen karar";
                        const searchText = item.query
                            ? `${primary} içinde "${item.query}" için arandı${
    ...
    const renderEvent = (
            ...
            return (
                ...
                >
                    ...
                    <div className="ml-2 min-w-0">
                        <span className="font-medium">
                            {event.isStreaming ? "Bağlantı kullanılıyor..." : label}
                        </span>
                        {isError && event.error && (
        ...
        if (event.type === "courtlistener_search_case_law") {
            ...
            const detail = event.isStreaming
                ...
                : event.error
                  ? event.error
                  : `${count} ${count === 1 ? "sonuç" : "sonuç"}${event.query ? ` ("${event.query}" için)` : ""}`;
            return (
                <CourtListenerBlock
                    key={globalIdx}
                    label={
        ...
        if (event.type === "courtlistener_get_cases") {
            ...
            const items: CourtListenerBlockItem[] =
                ...
                event.cluster_ids.map((clusterId) => {
                    ...
                    return {
                        ...
                        citation: citation?.citation ?? `Küme ${clusterId}`,
            ...
            return (
                <CourtListenerBlock
                    ...
                    label={
                        event.isStreaming
                            ? `${displayLabel} getiriliyor`
                            : event.error
                              ? "Karar getirme başarısız"
                              : `${displayLabel} getirildi`
                    }
                    detail={detail}
                    isStreaming={!!event.isStreaming}
                    hasError={!!event.error}
        ...
        if (event.type === "courtlistener_find_in_case") {
            ...
            if (searches.length > 0) {
                ...
                const searchLabel = `${caseCount} ${caseCount === 1 ? "kararda" : "kararda"} ${searches.length} ${
                    searches.length === 1 ? "arama" : "arama"
                }`;
                const detail = event.isStreaming
                    ? undefined
                    : event.error
                      ? event.error
                      : `(${matches} ${matches === 1 ? "eşleşme" : "eşleşme"})`;
                ...
                return (
                    <CourtListenerBlock
                        ...
                        label={
                            event.isStreaming
                                ? `${searchLabel} çalıştırılıyor`
                                : event.error
                                  ? "Karar aramaları başarısız"
                                  : `${searchLabel} çalıştırıldı`
                        }
                        detail={detail}
                        isStreaming={!!event.isStreaming}
                        hasError={!!event.error}
            ...
            const caseLabel =
                ...
                (event.cluster_id ? `küme ${event.cluster_id}` : "karar");
            const detail = event.isStreaming
                ? event.query
                    ? `${caseLabel} içinde "${event.query}" için`
                    : caseLabel
                : event.error
                  ? event.error
                  : `${caseLabel} içinde ${matches} ${matches === 1 ? "eşleşme" : "eşleşme"}${event.query ? ` ("${event.query}" için)` : ""}`;
            return (
                <CourtListenerBlock
                    key={globalIdx}
                    label={
        ...
        if (event.type === "courtlistener_read_case") {
            ...
            return (
                <CourtListenerBlock
                    ...
                    label={
                        event.isStreaming
                            ? `${caseLabel} kararı okunuyor`
                            : event.error
                              ? `Karar okuma başarısız: ${caseLabel}`
                              : `${caseLabel} kararı okundu`
                    }
                    detail={detail}
                    isStreaming={!!event.isStreaming}
                    hasError={!!event.error}
        ...
        if (event.type === "courtlistener_verify_citations") {
            ...
            const citationLabel = `${citations} ${citations === 1 ? "atıf" : "atıf"}`;
            const detail = event.isStreaming
                ? undefined
                : event.error
                  ? event.error
            ...
            return (
                <CourtListenerBlock
                    ...
                    label={
                        event.isStreaming
                            ? `${citationLabel} doğrulanıyor`
                            : event.error
                              ? "Atıf doğrulama başarısız"
                              : `${citationLabel} doğrulandı`
                    }
                    detail={detail}
                    isStreaming={!!event.isStreaming}
                    hasError={!!event.error}
