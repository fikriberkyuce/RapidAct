...
export function ProjectDetailsModal({
    ...
    const accessLabel =
        Array.isArray(project.shared_with) && project.shared_with.length > 0
            ...
            : "Özel";
    const isPrivateOwnedProject =
        project.is_owner !== false && accessLabel === "Özel";
    const ownerLabel =
    ...
    async function handleSave() {
        ...
        try {
            ...
            setError("Proje ayrıntıları güncellenemedi.");
        } finally {
            setSaving(false);
        }
    }
    ...
    return (
        <Modal
            open={open}
            onClose={onClose}
            breadcrumbs={["Projeler", project.name, "Ayrıntılar"]}
            secondaryAction={{
            ...
            <div className="flex flex-col gap-5 py-1">
                ...
                <div className="flex flex-col gap-3">
                    ...
                    <input
                        ...
                        disabled={!canEdit || saving}
                        placeholder="Dosya No yok"
                        className="h-9 w-full rounded-md border border-gray-200 bg-gray-50 px-3 text-sm text-gray-900 outline-none transition-colors focus:border-gray-300 disabled:cursor-not-allowed disabled:text-gray-400"
                    />
                </div>

                <div className="divide-y divide-gray-100 text-sm">
                    <DetailRow label="Sahiplik" value={accessLabel} />
                    <DetailRow
                        label="Sahip"
                        value={
