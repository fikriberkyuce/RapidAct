    ...
    return (
        <div className="flex-1 flex flex-col min-h-0">
            <div className="flex items-center h-8 pr-8 border-b border-gray-200 text-xs text-gray-500 font-medium select-none shrink-0">
                ...
                <div className="w-20 shrink-0 text-left">Sürüm</div>
                <div className="w-32 shrink-0 text-left">Oluşturulma</div>
                <div className="w-32 shrink-0 text-left">Güncellenme</div>
...
export function ProjectDocumentsView({ projectId }: Props) {
    ...
    async function replaceVersionFile(
        ...
        if (replaced) {
            setViewingDocVersion({
                id: replaced.id,
                label: replaced.filename?.trim() || "Sürüm",
            });
        }
    }
        ...
        try {
            ...
            setViewingDocVersion(
                nextVersion
                    ? {
                          id: nextVersion.id,
                          label: nextVersion.filename?.trim() || "Sürüm",
                      }
                    : null,
            );
        } catch (e) {
            console.error("deleteDocumentVersion failed", e);
            setDocumentRenameWarning("Bu sürüm silinemedi.");
        }
    }
    ...
    async function confirmDeletePendingFolder() {
        ...
        try {
            ...
            setProjectActionWarning(
                "Klasör silinemedi. Lütfen tekrar deneyin.",
            );
        }
    }
    ...
    async function handleRemoveDoc(docId: string) {
        ...
        if (doc && user?.id && doc.user_id && doc.user_id !== user.id) {
            setOwnerOnlyAction("bu belgeyi silme");
            return;
        }
    ...
    async function handleDeleteSelectedDocs() {
        ...
        if (failedCount > 0) {
            setProjectActionWarning(
                `${failedCount} belge silinemedi. Lütfen tekrar deneyin.`,
            );
        }
        if (blocked > 0) {
            setOwnerOnlyAction(
                `seçili belgelerden ${blocked} tanesini silme — yalnızca belgeyi oluşturan kişi silebilir`,
            );
        }
    }
    ...
    async function saveExistingDocumentAsNewVersion(
        ...
        try {
            ...
            setProjectActionWarning(
                apiErrorDetail(err) ??
                    "Bu belge yeni sürüm olarak kaydedilemedi.",
            );
        } finally {
            setUploadingVersionDocIds((prev) => {
                const next = new Set(prev);
                                            ...
                                            <div className="w-8 shrink-0 flex justify-end">
                                                {!isProcessing && (
                                                    <RowActions
                                                        ...
                                                        renameLabel="Belgeyi yeniden adlandır"
                                                        onDownload={() =>
                                                            downloadDoc(doc.id)
                                                        }
                                                        onShowAllVersions={
    ...
    const actionsDropdown =
        selectedDocIds.length > 0 ? (
            <div ref={actionsRef} className="relative">
                ...
                {actionsOpen && (
                    <div className="absolute top-full right-0 mt-1 w-44 rounded-lg border border-gray-100 bg-white shadow-lg z-[120] overflow-hidden">
                        <button
    ...
    const toolbarActions = (
        <div className="flex items-center gap-5">
            ...
            >
                ...
                <span className="hidden sm:inline">Alt Klasör Ekle</span>
            </button>
            <button
    ...
    const pendingVersionDropMessage = pendingVersionDrop ? (
        <div className="space-y-2">
            <p>
                ...
                {" "}dosyasının yeni sürümü olarak kaydetmek üzeresiniz.
            </p>
            <p>
                <span className="font-medium text-gray-950">
                    {pendingVersionDrop.sourceDoc.filename}
                </span>
                {" "}artık ayrı bir belge olarak var olmayacak
                {(currentVersionNumber(pendingVersionDrop.sourceDoc) ?? 1) > 1
                    ? " ve eski sürümleri silinecek"
                    : ""}
                .
            </p>
        </div>
    ...
    const pendingDeleteDocMessage = pendingDeleteDoc ? (
        <div className="space-y-2">
            <p>
                ...
                {" "}belgesinin {pendingDeleteDocVersionCount} sürümü var.
                Bu belgeyi silmek tüm sürümlerini de silecektir.
            </p>
        </div>
    ) : undefined;
    const pendingDeleteFolderMessage = pendingDeleteFolder ? (
        <div className="space-y-2">
            <p>
                ...
                {" "}kalıcı olarak silinecek, buna{" "}
                <span className="font-medium text-gray-950">
                    {pendingDeleteFolder.folder.name}
                </span>
                {pendingDeleteFolder.folderIds.length > 1
                    ? " ve iç içe alt klasörleri"
                    : ""}{" "}
                dahil.
            </p>
            {pendingDeleteFolder.documentCount > 0 && (
                <p>
                    ...
                    {pendingDeleteFolder.folderIds.length === 1
                        ? "klasördeki"
                        : "klasörlerdeki"}{" "}
                    {pendingDeleteFolder.documentCount} belge de kalıcı olarak silinecektir.
                </p>
            )}
        </div>
    ) : undefined;
    ...
    return (
        <div className="relative flex h-full min-h-0 flex-1 flex-col overflow-hidden">
            ...
            <ConfirmPopup
                ...
                title="Yeni sürüm olarak kaydedilsin mi?"
                                ...
                                uploadingDroppedFilenames.length === 0 ? (
                                    ...
                                    >
                                        ...
                                        <p className="text-sm text-gray-400">
                                            PDF, DOCX veya DOC dosyalarını buraya bırakın
                                        </p>
                                    </div>
                                ) : (
                                    <div
                                            ...
                                            >
                                                {menuDoc ? (
                                                    ...
                                                    <RowActionMenuItems
                                                        ...
                                                        renameLabel="Klasörü yeniden adlandır"
                                                        onDelete={
                                                            contextMenu.showFolderActions &&
                                                            contextMenu.folderId
                                                                ? () =>
...
function extensionChangeWarning(filename: string) {
    ...
    return extension
        ? `Dosya uzantıları burada değiştirilemez. Adın sonunda ${extension} uzantısını koruyun.`
        : "Dosya uzantıları burada değiştirilemez.";
}
