...
export function WorkflowDetailsModal({
    ...
    const typeLabel = workflow.type === "tabular" ? "Tablo" : "Asistan";
    ...
    const ownerLabel =
        workflow.is_owner === false
            ? workflow.shared_by_name?.trim() || "Bilinmiyor"
    ...
    async function handleSave() {
        ...
        try {
            ...
            setError("İş akışı ayrıntıları güncellenemedi.");
        } finally {
            setSaving(false);
        }
    }
    ...
    return (
        <Modal
            open={open}
            onClose={onClose}
            breadcrumbs={["İş Akışları", workflow.title, "Ayrıntılar"]}
            secondaryAction={
                canShare
                    ? {
                          label: "İş Akışını Paylaş",
            ...
            footerStatus={
                error ? (
                    ...
                    <span className="text-sm text-gray-400">Güncellendi</span>
                ) : null
            }
            primaryAction={
                canEdit
                    ? {
                          label: saving ? "Güncelleniyor..." : "Güncelle",
        ...
        >
            <div className="flex flex-col gap-5 py-1">
                ...
                <div className="divide-y divide-gray-100 text-sm">
                    <DetailRow label="Tür" value={typeLabel} />
                    <DetailRow
                        label="Sahiplik"
                        value={
                            sharesLoading ? (
                    ...
                    />
                    <DetailRow label="Sahip" value={ownerLabel} />
                </div>
            </div>
        </Modal>
    );
