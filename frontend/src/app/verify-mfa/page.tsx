        ...
        async function loadMfaState() {
            ...
            try {
                ...
                if (verified.length === 0) {
                    setError(
                        "Bu hesap için doğrulanmış bir kimlik doğrulayıcı bulunmuyor.",
                    );
                }
            } catch (loadError) {
                if (cancelled) return;
                setError(
                    loadError instanceof Error
                        ? loadError.message
                        : "Kimlik doğrulayıcı doğrulaması yüklenemedi.",
                );
            } finally {
            ...
            <div className={`w-full max-w-md ${authGlassCardClassName}`}>
                <div className="mb-8 space-y-2">
                    ...
                    <p className="text-sm text-gray-500">
                        Devam etmek için kimlik doğrulayıcı uygulamanızdaki altı
                        haneli kodu girin.
                    </p>
                </div>
                ...
                <div className="space-y-6">
                    {loading ? (
                        <div className="flex h-13 items-center justify-center text-sm text-gray-500">
                            ...
                            Kimlik doğrulayıcı yükleniyor...
                        </div>
                    ) : factors.length === 0 ? (
                        <p className="rounded-lg bg-gray-100 px-3 py-2 text-sm text-gray-600">
                            Bu oturum için doğrulanmış bir kimlik doğrulayıcı
                            bulunmuyor.
                        </p>
                    ) : (
                        <>
