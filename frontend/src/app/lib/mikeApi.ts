...
async function apiRequest<T>(path: string, init?: RequestInit): Promise<T> {
    ...
    return normalizeUnicode(await response.json()) as T;
}

function normalizeUnicode<T>(value: T): T {
    if (typeof value === "string") return value.normalize("NFC") as T;
    if (Array.isArray(value)) return value.map(normalizeUnicode) as T;
    if (value && typeof value === "object") {
        const out: Record<string, unknown> = {};
        for (const [k, v] of Object.entries(value)) {
            out[k] = normalizeUnicode(v);
        }
        return out as T;
    }
    return value;
