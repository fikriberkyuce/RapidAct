...
export default function ProjectAssistantChatPage({ params }: Props) {
    ...
    async function handleDeleteChat() {
        if (chatOwnerId && user?.id && chatOwnerId !== user.id) {
            setOwnerOnlyAction("bu sohbeti silme");
            return;
        }
    ...
    async function handleRenameChat() {
        if (chatOwnerId && user?.id && chatOwnerId !== user.id) {
            setOwnerOnlyAction("bu sohbeti yeniden adlandırma");
            return;
        }
        ...
        <div className="flex flex-col h-full">
            ...
            <PageHeader
                ...
                breadcrumbs={[
                    ...
                    chatLoaded
                        ? {
                              label: chatTitle ?? "Adsız Yeni Sohbet",
                          }
                        : {
                              loading: true,
                            ...
                            <div className="flex items-center justify-center h-full px-8 bg-gray-100">
                                <div className="text-center space-y-3">
                                    ...
                                    <p className="font-serif text-base text-gray-500">
                                        İpucu: Asistana okuması veya düzenlemesi
                                        için Gezginden bir belgeyi sürükleyip
                                        bırakabilirsiniz.
                                    </p>
                                </div>
                            </div>
