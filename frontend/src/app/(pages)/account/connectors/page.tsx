...
function parseCustomHeaders(raw: string): Record<string, string> | undefined {
    ...
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
        throw new Error("Özel başlıklar bir JSON nesnesi olmalıdır.");
    }
    const headers: Record<string, string> = {};
    for (const [key, value] of Object.entries(parsed)) {
        if (typeof value !== "string") {
            throw new Error("Özel başlık değerleri metin (string) olmalıdır.");
        }
        headers[key] = value;
    }
    return headers;
...
export default function ConnectorsPage() {
    ...
    const loadConnectors = useCallback(async () => {
        ...
        try {
            ...
            setError(
                err instanceof Error ? err.message : "Bağlantılar yüklenemedi.",
            );
        } finally {
            setLoading(false);
        }
    ...
    const connectConnectorOAuth = async (
        ...
        if (!authorizationUrl) {
            popup?.close();
            throw new Error("OAuth yetkilendirme URL'si döndürülmedi.");
        }
        if (!popup) {
        ...
        await new Promise<void>((resolve, reject) => {
            const timeout = window.setTimeout(() => {
                cleanup();
                reject(new Error("OAuth yetkilendirmesi zaman aşımına uğradı."));
            }, 5 * 60 * 1000);
            const poll = window.setInterval(() => {
                if (popup.closed) {
                    cleanup();
                    reject(new Error("OAuth yetkilendirme penceresi kapatıldı."));
                }
            }, 700);
            const cleanup = () => {
            ...
            const onMessage = (event: MessageEvent<McpOAuthPopupMessage>) => {
                ...
                reject(
                    new Error(
                        event.data.detail || "OAuth yetkilendirmesi başarısız oldu.",
                    ),
                );
            };
            ...
            try {
                ...
                try {
                    ...
                    if (
                        ...
                        setAddAuthMessage(
                            "Bu MCP sunucusuna bağlanmayı tamamlamak için açılır penceredeki yetkilendirmeyi tamamlayın.",
                        );
                        setAddStep("auth");
                ...
                if (isGoogleMcpConnector(refreshed) && !refreshed.oauthConnected) {
                    setAddAuthMessage(
                        "Bu MCP sunucusuna bağlanmayı tamamlamak için açılır pencerede Google'ı yetkilendirin.",
                    );
                    setAddStep("auth");
            ...
            <div className="grid grid-cols-[minmax(0,1fr)_auto] gap-x-3 gap-y-3">
                <div className="min-w-0 text-left">
                    <h3 className="flex min-w-0 items-center gap-2 text-sm font-semibold text-gray-900">
                        ...
                        <span className="shrink-0 text-xs font-medium text-gray-500">
                            {toolCount} {toolCount === 1 ? "araç" : "araç"}
                        </span>
                    </h3>
                </div>
                <div
                ...
                >
                    <AccountToggle
                        ...
                        label={connector.enabled ? "Etkin" : "Devre dışı"}
                        onChange={(enabled) =>
    ...
    return (
        <Modal
            ...
            cancelAction={
                step === "working" || step === "auth"
                    ? false
                    : { label: step === "success" ? "Tamam" : "İptal", onClick: onClose }
            }
            footerStatus={
                error ? (
        ...
        >
            {step === "success" && result ? (
                ...
                <div className="space-y-4 pb-4">
                    <p className="text-sm text-gray-500">
                        Asistan, bu MCP sunucusuna ve etkin araçlarına
                        erişebilecek.
                    </p>
                    <ConnectorForm
                        draft={draft}
...
function McpConnectorDetailsModal({
    ...
    return (
        <Modal
            open={!!connector}
            onClose={onClose}
            breadcrumbs={["Bağlantılar", connector?.name ?? "MCP bağlantısı"]}
            headerAction={
                connector ? (
                    <AccountToggle
                        checked={connector.enabled}
            ...
            cancelAction={{ label: "Kapat", onClick: onClose }}
            footerStatus={
                error ? (
                ...
                <div className="min-w-0">
                    <div className="relative">
                        ...
                        {draft.bearerToken && (
                            <button
                                ...
                                aria-label={
                                    showToken ? "Token'ı gizle" : "Token'ı göster"
                                }
                                disabled={disabled}
                            >
                                {showToken ? (
                    ...
                    {showTokenNote && (
                        <p className="mt-1 text-right text-xs text-gray-500">
                            Token'lar şifreli olarak saklanır.
                        </p>
                    )}
                </div>
            </div>
                ...
                {showAdvanced && (
                    <label className="grid gap-2 sm:grid-cols-[96px_minmax(0,1fr)] sm:items-start">
                        ...
                        <div className="min-w-0">
                            ...
                            />
                            <p className="mt-1 text-right text-xs text-gray-500">
                                Gizli bilgiler şifreli olarak saklanır.
                            </p>
                        </div>
                    </label>
                )}
            ...
            <div className="flex items-start gap-3 rounded-xl border border-green-100/80 bg-green-50/80 px-3 py-3 text-green-800 shadow-[0_3px_9px_rgba(15,23,42,0.03),inset_0_1px_0_rgba(255,255,255,0.9),inset_0_-4px_9px_rgba(255,255,255,0.05)] backdrop-blur-xl">
                ...
                <div className="min-w-0">
                    <p className="truncate text-sm font-medium">
                        {connector.name} bağlandı.{" "}
                        <span className="font-normal text-green-700">
                        {connector.tools.length} araç bulundu.
                        </span>
                    </p>
                </div>
            </div>
