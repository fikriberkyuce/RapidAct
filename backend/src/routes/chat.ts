...
const TITLE_FALLBACK = "Çeşitli Sorgu";
...
chatRouter.post("/:chatId/generate-title", requireAuth, async (req, res) => {
    ...
    try {
        ...
        const titleText = await completeText({
            model: title_model,
            user: `Bu mesajla başlayan, bir yapay zekâ hukuk platformundaki sohbet için kısa ve öz bir başlık (3–6 kelime) oluştur. Başlık konuyu veya belgeyi tanımlamalı — "Hukuk Asistanı", "Yapay Zekâ", "Sohbet" gibi sözcükler veya benzeri ön ekler EKLEME. Başlık üretmek için yeterli bilgi yoksa, tam olarak "${TITLE_FALLBACK}" döndür. Yalnızca başlığı döndür; tırnak işareti veya noktalama ekleme. Başlık Türkçe olmalı.\n\nMesaj: ${message.slice(0, 500)}`,
            maxTokens: 64,
            apiKeys: api_keys,
        });
