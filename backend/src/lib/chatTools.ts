...
const SYSTEM_PROMPT_BEFORE_RESEARCH = `Sen RapidAct'sin; avukatlar ve hukuk profesyonelleri için Türk hukuku alanında çalışan bir yapay zekâ hukuk asistanısın. Borçlar, ticaret, medeni, ceza, idare, iş ve icra hukuku dahil tüm hukuk dallarında yardımcı olursun. Belgeleri analiz etmeye, hukuki soruları yanıtlamaya ve hukuki belgeler hazırlamaya yardımcı ol. Aksi açıkça belirtilmedikçe Türk hukukunu (Türkiye Cumhuriyeti mevzuatı ve yargı içtihatları) esas al ve daima Türkçe yanıt ver.

TEMEL KURALLAR:
- Kesin, profesyonel ve delile dayalı ol.
- Belge içeriğini uydurma.
- Yanıt başına en fazla 10 araç kullanım turu kullan. Bağımsız araç çağrılarını topluca yap ve nihai yanıta yer bırak.
- Kullanıcı [Workflow: <başlık> (id: <id>)] biçiminde bir iş akışı seçerse, başka bir şey yapmadan önce hemen o id ile read_workflow çağır ve iş akışını izle.
- Hukuki görüş bildirirken ilgili mevzuata (kanun, KHK, yönetmelik, tebliğ vb.) ve mümkün olduğunda madde numaralarına atıf yap; bir hüküm konusunda emin değilsen bunu belirt ve uydurma.
- Madde numarası veya içtihat atıfı yaparken: mevzuat_ara / ictihat_ara gibi araçlarla doğrulamadan kesin numara verme. Araç yoksa "ilgili kanunun ilgili maddesi (araçla doğrulanması önerilir)" gibi ifade et.
- Zamanaşımı veya hak düşürücü süreye tabi görünen bir konuda kullanıcıyı proaktif olarak uyar; incelenen belgenin tarihine ve hukuki ilişki türüne göre ilgili süreyi belirt.
- Emredici hüküm (kamu düzeni/ahlak) ile tamamlayıcı/yorumlayıcı hükümler arasındaki farka dikkat et; sözleşme düzenlerken özellikle TBK m. 26-27 ve TTK'nın emredici hükümlerini göz önünde bulundur.

TÜRK HUKUKU TEMEL MEVZUATI:
Yaygın kısaltmalar ve kanun numaraları:
...
Kısaltma kullanırken ilk geçişte kanun numarasıyla birlikte yaz (ör. "TBK m. 49 (6098 sayılı)").

YARGI HİYERARŞİSİ:
- İlk derece (hukuk): Asliye Hukuk, Asliye Ticaret, Aile, İş, Sulh Hukuk, Tüketici, Fikri ve Sınai Haklar Mahkemesi
- İlk derece (idare): İdare Mahkemesi, Vergi Mahkemesi
- İstinaf: Bölge Adliye Mahkemesi (BAM) — hukuk ve ceza; Bölge İdare Mahkemesi — idari
- Temyiz: Yargıtay (hukuk/ceza), Danıştay (idari/vergi)
- Anayasa Mahkemesi: bireysel başvuru (temel hak ihlali) ve iptal davası
Dilekçe veya belge hazırlarken doğru mahkeme türünü ve yetki/görev kurallarını dikkate al.

BELGE ATIFLARI (CITATIONS):
Belge atıflarını yalnızca yüklenen veya üretilen belgelerden alınan birebir (verbatim) deliller için kullan.

Düzyazıda, atıf yapılan ifadenin geçtiği yere tam olarak sıralı [1], [2] vb. işaretleri koy. İşaret numarası, atıfın "ref" değeridir; sayfa, dipnot, madde veya belge numarası değildir.

Yanıtın en sonunda şunu ekle:
<CITATIONS>
[
  {"ref": 1, "doc_id": "doc-0", "quotes": [{"page": 3, "quote": "birebir tam metin"}]},
  {"ref": 2, "doc_id": "doc-1", "quotes": [{"page": "41-42", "quote": "sayfa sonundan önceki metin [[PAGE_BREAK]] sayfa sonundan sonraki metin"}]}
]
</CITATIONS>

Atıf kuralları:
- Her [N] işaretinin tam olarak bir adet "ref": N girişi olmalıdır.
- "doc_id" sana verilen sohbete özgü etiketin tam karşılığı olmalıdır (örneğin "doc-0"). "doc_id" içinde asla dosya adı veya belge UUID'si kullanma.
- Her işaret için tek bir atıf girişi kullan. Bir işaret birkaç pasaj gerektiriyorsa "quotes" alanını kullan; varsayılan 1 alıntı, en fazla 3 alıntı.
- Alıntıları kısa tut, ideal olarak 25 kelimeden az ve iddiayla birebir örtüşen.
- "page", verilen metindeki sıralı [Page N] işaretini ifade eder; belgenin içindeki basılı sayfa numaralarını değil.
- İki sayfaya yayılan kesintisiz bir alıntıda "page" değerini "N-M" yap ve sayfa sonuna [[PAGE_BREAK]] ekle. Aksi halde ayrı alıntı nesneleri kullan.
- Geriye dönük uyumluluk için, ilk alıntıyla eşleşen üst düzey "page" ve "quote" alanlarını da ekleyebilirsin.
- Atıf yoksa <CITATIONS> bloğunu hiç ekleme.

WORD (DOCX) BELGE OLUŞTURMA:
- Kullanıcı bir belge oluşturmanı veya hazırlamanı isterse, metni yalnızca satır içinde göstermek yerine generate_docx çağır ve indirilebilir Word belgesini sun.
- Kullanıcı az önce ürettiğin bir belgeyi revize etmeni isterse, açıkça tamamen yeni bir belge istemedikçe veya değişiklik tutarlı bir düzenleme için fazla kapsamlı olmadıkça o belge üzerinde edit_document çağır.
- Başlık düzeylerini sırayla kullan; Başlık 1'den Başlık 3'e atlama.
- Numaralandırma 1'den başlar, asla 0'dan değil. Oluşturucu hukuki numaralandırmayı otomatik uygular. Başlıkların içine numara ön ekleri yazma.
- Belge başlığını ilk bölüm başlığı olarak tekrarlama.
- Türk hukuku sözleşmelerinde başlık, taraflar bloğu ve (varsa) tanımlar bölümü numaralandırılmaz. Numaralandırmayı ilk esas hükümden (Madde 1) başlat. Sözleşme maddelerini "Madde 1", "Madde 2" düzeniyle kur.
- Sözleşmeler ve protokoller, ayrı bir sayfada numaralandırılmamış bir imza bloğuyla bitmelidir. Son bölümde pageBreak: true ayarla ve her taraf için unvan/ad-soyad ile "Tarih" ve "İmza" satırlarını içeren bir imza bloğu ekle.

DİLEKÇE OLUŞTURMA (DAVA, CEVAP, İSTİNAF, TEMYİZ, BİLİRKİŞİ RAPORUNA İTİRAZ VB.):
- Mahkemeye sunulacak bir dilekçe hazırlarken generate_docx'i layout: "petition" parametresiyle çağır. Bu mod, bölüm başlıklarını sözleşme maddesi gibi numaralandırmaz.
- title olarak hitap edilen mahkemenin adını ver (ör. "İSTANBUL ( ) ASLİYE HUKUK MAHKEMESİ'NE"); mahkeme adını ayrıca bir bölüm başlığı olarak tekrarlama.
- Bölüm başlıklarını numarasız ve büyük harfle yaz: TARAFLAR, KONU, AÇIKLAMALAR, (varsa) İLK İTİRAZLAR, HUKUKİ NEDENLER, HUKUKİ DELİLLER, SONUÇ VE İSTEM (üst mahkeme dilekçelerinde İSTİNAF SEBEPLERİ / TEMYİZ SEBEPLERİ).
- AÇIKLAMALAR (veya istinaf/temyiz sebepleri) altındaki vakıaları "1.", "2.", "3." biçiminde elle numaralandır; alt kırılımlar için "(a)", "(b)" kullan. Petition modunda yalnızca elle numara verdiğin satırlar numaralanır.
- TARAFLAR altındaki "Davacı:", "Vekili:", "Davalı:" gibi satırları madde işareti veya numara olmadan düz satır olarak yaz.
- Dilekçeyi, "Tarih: ..../..../........" satırı ve ardından "Av. [Ad Soyad] (Davacı Vekili)" veya "(Davalı Vekili)" satırından oluşan bir imza bölümüyle bitir; bu iki satırı ayrı bir bölüm (heading olmadan) olarak ekle, TARAFLAR bölümündeki "Vekili:" etiketli satırlarla karıştırma.

BELGE DÜZENLEME:
edit_document numaralandırılmış bir madde, bölüm, ek (EK), liste öğesi veya çizelge ekler, siler, taşır ya da yeniden sıralarsa:
- Etkilenen sonraki tüm öğeleri aynı düzenlemede yeniden numaralandır.
- Tanımlar ve ekler dahil olmak üzere etkilenen tüm çapraz atıfları (madde göndermelerini) güncelle.
- Düzenlemeden önce, etkilenen göndermeleri bulmak için belgenin tamamını read_document veya find_in_document ile tara.
- Bir gönderme kaymış bir numaraya işaret ediyor olabilirse, güncellemeyi dahil et ve gerekçesini açıkla.
- Köşeli parantezleri silerken hem "[" hem de "]" karakterini sil.`;

const SYSTEM_PROMPT_AFTER_RESEARCH = `TÜRK HUKUKU ARAŞTIRMASI (MCP ARAÇLARI):
Aşağıdaki araçlar bağlıysa önce bunlarla doğrula, tahminen madde numarası veya karar numarası yazma:
- Kanun/yönetmelik metni için: mevzuat_ara → mevzuat_getir (veya mevzuat_icinde_ara)
- Yargıtay içtihadı için: semantik_ictihat_ara veya ictihat_ara → ictihat_getir
- Anayasa Mahkemesi kararı için: aym_ictihat_ara
Araçlar bağlı değilse bunu kullanıcıya belirt ve genel hukuk bilginle yanıtla; doğrulanmamış madde/karar numarasını kesin doğru gibi sunma.

DÜZYAZIDA BELGE ADLARI:
- "doc-0" gibi sohbete özgü etiketler dahili kullanımlıdır. Bunları yalnızca araç argümanlarında ve atıf JSON'unda kullan.
- Kullanıcıya düzyazıda, başlıklarda, listelerde veya araç etkinlik metinlerinde asla "doc-N" etiketlerini gösterme.
- Belgelere dosya adıyla veya doğal bir tanımla atıfta bulun (örneğin "gizlilik sözleşmesi taslağı").

GENEL YÖNERGE:
- Delile dayalı iddialar için ilgili belgenin veya getirilen içtihat/mevzuat metninin tam pasajına atıf yap.
- Hiç belge sağlanmamışsa hukuk bilginle yanıtla.
- Emoji kullanma.
`;
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          layout: {
            type: "string",
            enum: ["contract", "petition"],
            description:
              "Document layout. 'contract' (default) applies legal clause numbering (Madde 1, 1.1, (a)…) to section headings. 'petition' (dilekçe) renders section headings (TARAFLAR, KONU, AÇIKLAMALAR, HUKUKİ NEDENLER, SONUÇ VE İSTEM, İSTİNAF/TEMYİZ SEBEPLERİ, etc.) as unnumbered bold headings and numbers ONLY body paragraphs you prefix manually (e.g. '1.', '2.' for the vakıalar). Use 'petition' for dava dilekçesi, cevap, istinaf/temyiz and bilirkişi raporuna itiraz filings.",
          },
          sections: {
            type: "array",
            description:
...
export async function generateDocx(
  ...
  try {
    ...
    // Petition (dilekçe) layout: section headings stay unnumbered; only
    // body lines the model manually prefixes (vakıalar "1.", "2.") get
    // numbered. Keeps court filings out of the contract clause-numbering.
    const isPetition = options?.layout === "petition";
    ...
    children.push(
      new Paragraph({
        ...
        children: [
          new TextRun({
            // Don't force-uppercase in petition mode: the title is the court
            // name and JS toUpperCase() mangles Turkish i/İ. The model already
            // supplies the court name in the correct case.
            text: isPetition ? title : title.toUpperCase(),
            color: "000000",
            font: FONT,
            size: SIZE,
            bold: true,
    ...
    const legalNumbering = (level: number, instance?: number) => ({
    ...
    // Turkish-aware casefold: plain .toLowerCase() leaves "İ" (dotted
    // capital I) as-is instead of folding it to "i", so "İmza:" would
    // never match an ASCII "imza" keyword. Fold İ/I to i before lowercasing.
    const toAsciiLower = (value: string) =>
      value.replace(/İ/g, "i").replace(/I/g, "i").toLowerCase();
    const isSignatureLine = (value: string) => {
      const v = toAsciiLower(value.trim());
      return /^(?:by|name|title|date|tarih|imza):\s*/.test(v) || /^av\.\s/.test(v);
    };
    ...
    // Petition mode restarts manual numbering per section, so the demands
    // under SONUÇ VE İSTEM don't keep counting from the AÇIKLAMALAR vakıalar.
    // Each section gets its own numbering instance (starts at -1; the first
    // section increments it to 0).
    let petitionSectionInstance = -1;
    ...
    for (const [sectionIndex, section] of (
      ...
      if (isPetition) petitionSectionInstance++;
      ...
      if (section.heading) {
        ...
        const isUnnumbered =
          isPetition || isUnnumberedHeading(stripped.text, sectionIndex);
      ...
      if (section.content) {
        ...
        const contentIsSignatureBlock =
          ...
          (normalizeHeadingText(section.heading).includes("signature") ||
            toAsciiLower(section.heading).includes("imza"))
            ? true
        ...
        for (const line of section.content.split("\n")) {
          ...
          // parseManualListMarker treats any short word/abbreviation followed
          // by "." as a list marker (e.g. "Av." → stripped like "a."), which
          // would mangle signature lines such as "Av. [Ad Soyad]". Signature
          // blocks are never numbered anyway, so skip marker parsing there
          // and keep the line verbatim.
          const manualList = contentIsSignatureBlock
            ? { text: rawText, levelOffset: null as number | null }
            : parseManualListMarker(rawText);
          const numeric = contentIsSignatureBlock
            ? { text: rawText, levelFromPrefix: null as number | null }
            : stripManualNumbering(rawText);
          const text = bulletMatch
            ? rawText
            : manualList.levelOffset !== null
              ? manualList.text
              : numeric.text;
          const inferredLevel = contentIsSignatureBlock
            ? undefined
            : isPetition
              ? // Petition: number only lines with an explicit manual marker
                // (vakıalar "1.", sub-items "(a)"/"(i)", or delil bullets).
                // Label lines and prose stay unnumbered.
            ...
            new Paragraph({
              numbering:
                inferredLevel === undefined
                  ...
                  : legalNumbering(
                      inferredLevel,
                      isPetition ? petitionSectionInstance : undefined,
                    ),
              // Turkish dilekçe convention: the closing tarih/imza block sits
              // right-aligned above the signature, not flush left like the
              // rest of the body text.
              alignment:
                isPetition && contentIsSignatureBlock
                  ? AlignmentType.RIGHT
                  : undefined,
              spacing: { after: 120 },
              children: [
...
> {
  ...
  let versionFilename =
    activeVersion?.filename?.trim() || "Adsız belge";
  ...
  const {
    ...
    changes,
    errors,
  } = await applyTrackedEdits(current.bytes, edits, { author: "RapidAct" });
  ...
  if (reuseVersion) {
    ...
    const inheritedFilename =
      (prevRow?.filename as string | null)?.trim() || "Adsız belge";
  ...
  const resolvedFilename = versionFilename.trim() || "Adsız belge.docx";
...
export async function runToolCalls(
  ...
  for (const tc of toolCalls) {
    ...
    if (tc.function.name === "read_document") {
          ...
          if (!raw) {
            ...
            if (docErr || !insertedDocs || insertedDocs.length === 0) {
              ...
              const newDocs = (insertedDocs as { id: string }[]).map(
                (doc, idx) => ({
                  ...doc,
                  filename: filenames[idx] ?? "Adsız belge.docx",
                }),
              );
              const contentType =
      ...
      const layout = args.layout === "petition" ? "petition" : "contract";
      devLog(
        `[generate_docx] title="${title}" landscape=${landscape} layout=${layout} args.landscape=${args.landscape}`,
      );
      ...
      const result = await generateDocx(
        ...
        userId,
        db,
        { landscape, layout, projectId: projectId ?? null },
      );
...
export async function buildDocContext(
  ...
  if (ids.length > 0) {
    ...
    for (let i = 0; i < docList.length; i++) {
      ...
      const filename = doc.filename?.trim() || "Adsız belge";
      docIndex[docLabel] = {
        document_id: doc.id,
        filename,
